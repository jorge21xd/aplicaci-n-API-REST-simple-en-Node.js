...
Veamos paso a paso lo que va a ocurrir cuando ejecutemos este código. Primero se creará el __contexto de ejecución global__, en esta etapa el interprete guardará espacio para la declaración de la función __saludar__. Luego, cuando se encuentra con la invocación a la función __saludar__, va a crear un nuevo contexto, y como vemos dentro de ese contexto la variable saludo va a tomar el valor que le pasamos por parámetro:`'Hola'`. El stack quedaría cómo está representado en la primera parte de la figura de abajo.

![Closure](../_src/assets/Closure.png)
...
![Closure dos](../_src/assets/closure2.png)
...
![FunctionFactory](../_src/assets/functionFactory.png)
...
# Uso y Ejemplos de Métodos de Array en JavaScript

Vamos a explorar el uso de métodos de array en JavaScript, y también compararemos su utilidad con un enfoque tradicional utilizando bucles `for`. Utilizaremos el concepto de _closures_ que hemos discutido previamente para entender cómo afecta la ejecución de ciertos métodos de array.

### Métodos de Array

#### 1. **forEach()**
   - **Descripción:** Este método ejecuta una función dada una vez por cada elemento en el array.
   - **Ejemplo:**
     ```javascript
     var numbers = [1, 2, 3, 4];
...
#### 2. **map()**
   - **Descripción:** Crea un nuevo array con los resultados de llamar a una función proporcionada para cada elemento en el array.
   - **Ejemplo:**
     ```javascript
     var numbers = [1, 2, 3, 4];
...
#### 4. **reduce()**
   - **Descripción:** Aplica una función acumuladora a cada valor de un array, de izquierda a derecha, para reducirlo a un solo valor.
   - **Ejemplo:**
     ```javascript
     var numbers = [1, 2, 3, 4];
...
#### 5. **find()**
   - **Descripción:** Devuelve el primer elemento del array que cumple con la función de prueba proporcionada.
   - **Ejemplo:**
     ```javascript
     var numbers = [1, 2, 3, 4];
...
### Uso de Métodos de Array vs. For Tradicional

Vamos a comparar el uso de estos métodos con un enfoque tradicional utilizando un bucle `for`. Supongamos que queremos duplicar cada elemento de un array.
...
Ambos enfoques logran el mismo resultado, pero el uso de `map()` es más conciso y legible.